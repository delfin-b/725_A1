...
class GPT(nn.Module):
    ...
    def __init__(self, config, num_classes=3): # we have 3 classes for sentiment analysis
        ...
        # the LM head is removed since we need a classification head
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        # Copy matching weights (transpose attention projection weights if needed)
        transposed = ['attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fc.weight', 'mlp.c_proj.weight']
        sd_keys_hf = [k for k in sd_hf.keys() if not k.endswith('.attn.masked_bias') and not k.endswith('.attn.bias')]
        sd_keys = [k for k in sd.keys() if "classifier" not in k]  # exclude classifier head

        common_keys = list(zip(sd_keys_hf, sd_keys)) #####
        print(f"Loading {len(common_keys)} matched keys out of {len(sd_keys_hf)} HuggingFace keys and {len(sd_keys)} model keys.")


        for k1, k2 in zip(sd_keys_hf, sd_keys):
            if any(t in k1 for t in ['attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fc.weight', 'mlp.c_proj.weight']):
                sd[k2].copy_(sd_hf[k1].T)
            else:
                sd[k2].copy_(sd_hf[k1])
        return model
